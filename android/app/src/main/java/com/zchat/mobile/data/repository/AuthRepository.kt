...
class AuthRepository @Inject constructor(
    ...
    suspend fun login(username: String, password: String, rememberMe: Boolean): ApiResult<Unit> = apiCall {
    ...
    suspend fun register(username: String, email: String?, password: String, rememberMe: Boolean): ApiResult<Unit> = apiCall {
    ...
    suspend fun bootstrapSession(): Boolean {
        val session = tokenStore.memCache
        if (session.token.isNullOrBlank()) return false
        return try {
            authApi.me()
            true
        } catch (_: Exception) {
